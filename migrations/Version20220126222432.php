 ...
 */
final class Version20220126222432 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__order AS SELECT id, carinfo_id, client_id FROM "order"');
        $this->addSql('DROP TABLE "order"');
        $this->addSql('CREATE TABLE "order" (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, carinfo_id INTEGER NOT NULL, client_id INTEGER NOT NULL, CONSTRAINT FK_F5299398BCD0CA89 FOREIGN KEY (carinfo_id) REFERENCES carinfo (id) NOT DEFERRABLE INITIALLY IMMEDIATE, CONSTRAINT FK_F529939819EB6921 FOREIGN KEY (client_id) REFERENCES person (id) NOT DEFERRABLE INITIALLY IMMEDIATE)');
        $this->addSql('INSERT INTO "order" (id, carinfo_id, client_id) SELECT id, carinfo_id, client_id FROM __temp__order');
    ...
    public function down(Schema $schema): void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__order AS SELECT id, carinfo_id, client_id FROM "order"');
        $this->addSql('DROP TABLE "order"');
        $this->addSql('CREATE TABLE "order" (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, carinfo_id INTEGER NOT NULL, client_id INTEGER NOT NULL)');
        $this->addSql('INSERT INTO "order" (id, carinfo_id, client_id) SELECT id, carinfo_id, client_id FROM __temp__order');
        $this->addSql('DROP TABLE __temp__order');
        $this->addSql('DROP INDEX IDX_534E6880A35F2858');
        $this->addSql('CREATE TEMPORARY TABLE __temp__work AS SELECT id, _order_id, title FROM work');
